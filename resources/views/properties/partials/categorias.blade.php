    ...
    <div id="categorias-container">
        @if(old('categoria_ids') || isset($property))
            @php
                ...
                if (isset($property)) {
                    if($property instanceof \App\Models\PreapprovedProperty)
                        $pivots = \DB::table('category_preapproved_property_subcategories')
                        ->where('preapproved_property_id', $property->id)
                        ->get();
                    else
                                ...
                                <div>
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-danger"
                                                onclick="this.closest('.col-md-6').remove()">Remover
                                        </button>
                                    </div>
                                        ...
                                        @foreach($categoria->subcategories as $sub)
                                            <div class="form-check mt-2">
                                                ...
                                                <label class="form-check-label"
                                                       for="sub{{ $sub->id }}-{{ $categoria->id }}">{{ $sub->nome }}</label>
                                            </div>
                                        @endforeach
